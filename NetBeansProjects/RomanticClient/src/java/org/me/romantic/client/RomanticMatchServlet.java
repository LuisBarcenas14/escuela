 ...
 * @author HectorAdolfo
 */
@WebServlet(name = "RomanticMatchServlet", urlPatterns = {"/RomanticMatchServlet"})
public class RomanticMatchServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<title>Romantic Search</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Hey "+nombre+" these are the people you could date </h1>");
            ArrayList<Persona> lista = (ArrayList<Persona>) buscaCompatible(sexo,edad,signo);
            out.println("<table>");
    ...
    private java.util.List<org.me.Persona> buscaCompatible(java.lang.String sexo, int edad, java.lang.String signo) {
        // Note that the injected javax.xml.ws.Service reference as well as port objects are not thread safe.
        // If the calling of port operations may lead to race condition some synchronization is required.
        org.me.RomanticWebService port = service.getRomanticWebServicePort();
        return port.buscaCompatible(sexo, edad, signo);
    }
