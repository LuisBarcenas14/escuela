...
m1 l = if(length l)<2 then True else ((head l) == head (tail l)-1) && m1 (tail l)

m2 l = zipWith (<) l (tail l)

m3 Nulo = 0
m3 (Nodo _ Nulo Nulo)=0
m3 (Nodo r hi hd) = 1+m3 hi + m3 hd

llenaArbol n 
	| (n<1) =Nulo
	| otherwise = Nodo n (llenaArbol (n-5))(llenaArbol (n-7))


c = 
	(Nodo 10 (Nodo 50 (Nodo 60 (Nodo 3 Nulo Nulo) Nulo) Nulo) Nulo)