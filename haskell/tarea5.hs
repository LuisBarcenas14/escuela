...
ejemploArbol = Nodo 'a' 
  (Nodo 'b' (Nodo 'd' (Nodo 'h' Nulo Nulo) (Nodo 'i' Nulo Nulo)) 
  (Nodo 'e' (Nodo 'j' Nulo Nulo) (Nodo 'k' Nulo Nulo))) 
  (Nodo 'c' (Nodo 'f' (Nodo 'l' Nulo Nulo) (Nodo 'm' Nulo Nulo)) (Nodo 'g' (Nodo 'n' Nulo Nulo) (Nodo 'o' Nulo
  Nulo)))
...
depthFirstPath n t = dfs' n t []

dfs' n Nulo _ = []
dfs' n (Nodo r hi hd) c | n==r = c++[r]
                                | otherwise = (dfs' n (hi) (c++[r])++dfs' n (hd) (c++[r]))


breadthFirst arbol = bfs [arbol]

bfs [] = []
bfs (Nulo:xs) = bfs xs
bfs ((Nodo r hi hd):xs)= r:(bfs (xs++[hi,hd])) 

breadthFirstPath n arbol = dfs' n arbol []
{-
bfs' n [] c= []
bfs' n (Nulo:xs) c = bfs' n xs c
bfs' n ((Nodo r hi hd):xs) c | n==r = c++[r]
                             | otherwise = (bfs' n (xs++[hi,hd]) (c++[r]) ) 
-}
--SWI prolog

ejemploArbolSlope = Nodo ('a',0)
  (Nodo ('b',0)
...
calculaSlope Nulo = Nulo
calculaSlope (Nodo r hi hd) = (Nodo (r, (slope (Nodo r hi hd))) (calculaSlope hi) (calculaSlope hd) )
...
test3 = TestCase (assertEqual "Checking calculaSlope ejemploArbol" ejemploArbolSlope (calculaSlope ejemploArbol))
