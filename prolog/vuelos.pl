...
distanciaKm2([H1, H2], R):-
	calcDist([H1, H2], R).

distanciaKm2([H1, H2 | T], Sum):-
	calcDist([H1, H2], R),
	distanciaKm2([H2 | T], S2),
	Sum is R+S2.
%*/
%distancia2().
...
%duracionMin(_,0).
distanciaKm([X, Y], D):-vuelo(X, Y, _, D; _).
distanciaKm([[X, Y] | T], D):-vuelo(X, Y, _, D1, _), distanciaKm([Y, T], D2), D is D1+D2.

duracionMin([X, Y], D):-vuelo(X, Y, _, _, D).
duracionMin([[X, Y] | T], D):-vuelo(X, Y, _, _, D1), duracionMin([Y, T], D2), D is D1+D2.
